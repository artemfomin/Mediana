...
/// <summary>
/// UUIDv7 (RFC 9562): time-ordered, index-friendly (D10)
/// net10.0 — Guid.CreateVersion7(); netstandard2.1 —
/// (T-08 fix: RandomNumberGenerator System.Random)
/// </summary>
public static class GuidV7
{
    ...
    private static Guid Create()
    {
        lock (Lock)
        {
            ...
            // T-08: 74 RandomNumberGenerator
            Span<byte> random = stackalloc byte[10];
            RandomNumberGenerator.Fill(random);

            Span<byte> bytes = stackalloc byte[16];
            // Guid data1/data2 little-endian: timestamp mixed-endian
            // UUIDv7 (RFC big-endian )
            ...
            // 7 + 12 randA (random[0..1])
            bytes[7] = (byte)(0x70 | (random[0] & 0x0F));
            bytes[6] = random[1];

            // 10xx + 62 : (random[2..9]) + 16
