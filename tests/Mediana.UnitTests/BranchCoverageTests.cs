...
/// <summary> inthenbut byand: andand andandand, scoped-and, scan, andbutand, andand.</summary>
public class BranchCoverageTests
{
    ...
    // ── andbutand: no-op without ──────────────────────────────────────

    [Fact]
    public void Diagnostics_noop_and_enrich_tolerances()
    {
        // no-op in toand on but thenandand :
        // Enrich(null) — null-thenbut
        MedianaDiagnostics.Enrich(null, "k", "v");
        MedianaDiagnostics.Enrich(MedianaDiagnostics.StartDispatch("X"), "k", "v");
        // Listener-onandand — in MutationKillerTests (andfromand but thenand)
    }

    // ── andand: andand and scan ────────────────────────────────────────────
    ...
    {
        // thenin and CreateOrderHandler and AsyncCreateOrderHandler for CreateOrder —
        // scan by and (in and inand notthen MED001)
        var sc = new ServiceCollection();
    ...
    public void AddHandlersFromAssembly_ignores_generic_definitions()
    {
        ...
        // BCL not and handlers — freeze and without andand
    ...
    public async Task Stream_behavior_applies_when_matching()
    {
        var sc = new ServiceCollection()
            ...
            .AddMediana(c => c
                ...
                .AddMiddleware<CreateOrder, OrderCreated, OrderingBehavior>()); // not onand and
        var sp = sc.BuildServiceProvider();
    ...
    public async Task Send_wrong_response_type_throws()
    {
        ...
        // GetOrder andandin IQuery<OrderDto>, but IQuery<string> via Unsafe.As
    ...
    public async Task Stream_row_type_mismatch_throws()
    {
        ...
        // andandin row=int; andin Stream<OrderDto> via Unsafe.As
    ...
    {
        // AddMediana andand ; for inand and andthen in
    ...
    public async Task Missing_behavior_registration_throws_at_invoke()
    {
        var cfg = new MedianaConfiguration()
            .AddCommandHandler<AllocCommand, int, AllocCommandHandler>()
            .AddMiddleware<AllocCommand, int, AllocBehavior1>(); // behavior not in DI
        var mediator = new Mediator(cfg.Freeze(), new ServiceCollection().BuildServiceProvider());
    ...
    public async Task Event_behavior_missing_registration_throws()
    {
        var sc = new ServiceCollection()
            ...
            .AddMediana(c => c
                .AddEventHandler<OrderCreated, OrderCreatedAuditHandler>()
                .AddEventMiddleware<OrderCreated, EventOrderingBehavior>()); // not in DI
        var sp = sc.BuildServiceProvider();
        var mediator = sp.GetRequiredService<IMediator>();
    ...
    public async Task WithRegistry_extends_dispatch()
    {
        ...
        // runtime-toinand in copy-on-write
