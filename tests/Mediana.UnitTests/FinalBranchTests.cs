...
/// <summary>: mismatch-Mediator, command-, event GetRoot, registry race-branch.</summary>
public class FinalBranchTests
{
    ...
    public async Task Command_query_mismatch_typed_paths()
    {
        ...
        // SendExact<VC, string> — VC:ICommand<string> —
        // typed mediatr-typed : object-
    ...
    public async Task Event_getRoot_warm_path()
    {
        ...
        await mediator.Publish(new Ev()); // root
    ...
    {
        // entry EventCallSites → default-(Length == 0)
        var registry = Mediana.Dispatch.MessageRegistry.Empty.Add(
            typeof(Ev),
            new MessageEntry(HandlerKind.Event, typeof(Ev), null)); // call-sites
