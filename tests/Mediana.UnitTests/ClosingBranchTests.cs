...
/// <summary>: (null-, Diagnostics listener, scan, DI outbox).</summary>
public class ClosingBranchTests
{
    ...
    // 1) Scoped-: = null (RefResponse && !singleton) → InvokeAny SlowAny generic
    [Fact]
    ...
    // 4) Configuration: scan Mediana-+ + VoidResponse-
    [Fact]
    public void Configuration_scan_clean_assembly_and_policy_guard()
    {
        var cfg = new MedianaConfiguration()
            .AddHandlersFromAssembly(typeof(object).Assembly); // BCL:
        var registry = cfg.Freeze();
    ...
    // 6) Mediator: command-untyped-null fallback
    [Fact]
    public async Task Mediator_untyped_fallback_null_callsite_throws()
    {
        // entry CommandCallSite → mismatch-
    ...
    // 9) Relay StopAsync Start (graceful no-op )
    [Fact]
